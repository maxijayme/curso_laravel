...
@section('content')
    ...
    <div class="bg-gray-900 px-20 py-16 rounded-lg mb-8 relative overflow-hidden">
        ...
        <img src="{{ asset('images/dev.png') }}" class="absolute -right-4 -bottom-20 h-96  w-auto opacity-20 rounded-full">
    </div>
    <div class="px-4">
        <h1 class="text-xl mb-8 text-gray-800">Contenido técnico</h1>
        <div class="grid grid-cols-1 gap-4 mb-4">
            @foreach($posts as $post)
                <a href="{{route('post',$post->slug)}}" class="bg-gray-100 rounded-lg px-6 py-4">
                    <p class="text-xs flex items-center gap-2">
                        <span class="uppercase text-gray-700 bg-gray-200 rounded-full px-2 py-1">Tutotial</span>
